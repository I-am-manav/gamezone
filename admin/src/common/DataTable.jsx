...
export default function DataTable({ title, columns, data, loading, searchKeys = [], renderRow, headerAction, emptyMessage = "No data found" }) {
  ...
  const perPage = 7
  const filtered = data.filter(item => searchKeys.some(key => {
    let v = item; for (const k of key.split(".")) v = v?.[k]; return String(v || "").toLowerCase().includes(search.toLowerCase())
  }))
        ...
        <div className="table-responsive">
          <table className="table table-hover align-middle mb-0">
            <thead className="table-light">
              <tr><th style={{ width: 50 }}>#</th>{columns.map(c => <th key={c}>{c}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={columns.length + 1} className="text-center py-5">
                  <i className="ti-search-off" style={{ fontSize: 36, color: "#ccc" }} />
      ...
      {!loading && filtered.length > 0 && (
        <div className="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
          ...
          <ul className="pagination mb-0 pagination-sm">
            <li className={`page-item ${page === 1 ? "disabled" : ""}`}><button className="page-link" onClick={() => setPage(page - 1)}><i className="ti-chevron-left" /></button></li>
            ...
            <li className={`page-item ${page === totalPages ? "disabled" : ""}`}><button className="page-link" onClick={() => setPage(page + 1)}><i className="ti-chevron-right" /></button></li>
          </ul>
        </div>
      )}
    </div>
