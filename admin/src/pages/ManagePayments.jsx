  ...
  return (
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      ...
      {!loading&&(<div className="row g-3 mb-4">
        {[{l:"Total Revenue",v:`₹${revenue.toLocaleString("en-IN")}`,c:"#5D87FF"},{l:"Successful",v:payments.filter(p=>p.status==="Success").length,c:"#13DEB9"},{l:"Failed/Pending",v:payments.filter(p=>p.status!=="Success").length,c:"#FA896B"}].map(s=>(
      ...
      <DataTable title="All Payments" columns={["Customer","Amount","Razorpay Order ID","Payment ID","Date","Status"]}
        ...
        renderRow={(p,idx)=>(
          <tr key={p._id}>
            ...
            <td><code style={{fontSize:11}}>{p.razorpay_order_id?.slice(0,20)||"—"}</code></td>
