...
const Stars = ({rating}) => <div className="d-flex gap-1">{[1,2,3,4,5].map(s=><i key={s} className={`ti-star${s<=Math.round(rating)?"-filled":""}`} style={{color:"#FFAE1F",fontSize:14}}/>)}</div>
export default function ManageFeedbacks({ setIsAuthenticated, adminName }) {
  ...
  useEffect(()=>{getFeedbacks().then(r=>setFeedbacks(r.data.data||[])).catch(()=>toast.error("Failed")).finally(()=>setLoading(false))},[])
  ...
  return (
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      <div className="mb-4"><h4 className="fw-bold mb-1">Customer Feedbacks</h4><p className="text-muted mb-0">Monitor all game reviews and ratings.</p></div>
      {!loading&&(<div className="row g-3 mb-4">
        {[{l:"Total Reviews",v:feedbacks.length,c:"#5D87FF"},{l:"Avg Rating",v:`${avg} / 5`,c:"#FFAE1F"},{l:"5 Stars",v:feedbacks.filter(f=>f.rating===5).length,c:"#13DEB9"}].map(s=>(
      ...
      <DataTable title="All Feedbacks" columns={["Customer","Game","Rating","Review","Date"]}
        data={feedbacks} loading={loading} searchKeys={["user.name","game.name","feedback"]} emptyMessage="No feedbacks yet."
        renderRow={(f,idx)=>(
          <tr key={f._id}>
            <td>{idx}</td>
            <td><p className="mb-0 fw-semibold" style={{fontSize:13}}>{f.user?.name||"—"}</p><small className="text-muted">{f.user?.email}</small></td>
            <td><span style={{fontSize:13}}>{f.game?.name||"—"}</span></td>
            <td><Stars rating={f.rating}/><small className="text-muted">{f.rating}/5</small></td>
            <td><span style={{fontSize:13,color:"#697a8d"}}>"{f.feedback?.slice(0,80)}{f.feedback?.length>80?"...":""}"</span></td>
